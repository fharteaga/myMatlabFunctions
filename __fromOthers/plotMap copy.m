...
%
% I ADDED "SIGNATURE" to get more than 25K a day (get Secret from the same
% place you get the apikey
%
% USAGE:
...
showLabels = 1;
showPOI=1;
language = 'es-419'; % Spanish latam - https://developers.google.com/maps/faq#languagesupport
markeridx = 1;
markerlist = {};
styleInput='';
extraParams='';
...
if nargin >= 2
    for idx = 1:2:length(varargin)
        switch lower(varargin{idx})
            ...
            case 'style'
                styleInput = varargin{idx+1};
            case 'extraparams'
                extraParams = varargin{idx+1};
            case 'printurl'
                printURL = varargin{idx+1};
            case 'zoom'
                zoomlevel = varargin{idx+1};
            case 'apikey'
                apiKey = varargin{idx+1}; % set new key
            case 'secret'
                secret = varargin{idx+1}; % set new key
                % save key to file
...
% Construct query URL
preamble = 'https://maps.googleapis.com/maps/api/staticmap'; % s in https added by me!!  (Felipe)
...
if showPOI == 0
    poiStr = '&style=feature:poi|visibility:off';
else
    poiStr = '';
end

if ~isempty(styleInput)
    assert(showLabels==1)
    style = sprintf('&style=%s',styleInput);
else
    style = '';
end

if ~isempty(extraParams)
    extraParams = sprintf('&%s',extraParams);
else
    extraParams = '';
end
...
sensor = '&sensor=false';
url = [preamble location zoomStr sizeStr maptypeStr format markers poiStr labelsStr style languageStr sensor keyStr extraParams];
...
if(not(isempty(secret)))
        pv=pyenv;
        if(not(pv.Executable=="/opt/anaconda3/bin/python"))
            %pyenv('Version','/opt/anaconda3/bin/python'); %'/usr/bin/python3' 3.7,  '/usr/local/bin/python3' 3.9, '/opt/anaconda3/bin/python3' anaconda version
        end

        
        % Activate conda: $ . /opt/anaconda3/bin/activate && conda activate /opt/anaconda3
        % Check python installed: $ which -a python python3 

        %url=char(pyrunfile("urlsigner_modif.py","r",x=url,y=secret)); %
        %That is an equivalent version

        url=char(pyrunfile(sprintf("urlsigner_modif2.py '%s' '%s'",url,secret),'r'));
end
...
try
    ...
    websave(filepath,url); % Much faster than urlwrite!!!
...
if nargout <= 1 % plot map
    ...
    cax = caxis;
     ...
     if(alphaData<1)
        blanco=ones(size(uniImag));
        uniImag=((1-alphaData)*blanco+alphaData*uniImag);
     end
